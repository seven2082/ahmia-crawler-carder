...
# Initialize Elasticsearch client - handle both HTTP and HTTPS
_es_url = settings.ELASTICSEARCH_SERVER
_use_ssl = _es_url.startswith('https://')
_has_auth = bool(getattr(settings, 'ELASTICSEARCH_USERNAME', ''))
_has_ca = bool(getattr(settings, 'ELASTICSEARCH_CA_CERTS', ''))

if _use_ssl and _has_ca:
    # HTTPS with custom CA cert
    es_client = Elasticsearch(
        hosts=[_es_url],
        http_auth=(settings.ELASTICSEARCH_USERNAME, settings.ELASTICSEARCH_PASSWORD) if _has_auth else None,
    ...
    # HTTP or HTTPS without custom CA
    es_client = Elasticsearch(
        hosts=[_es_url],
        http_auth=(settings.ELASTICSEARCH_USERNAME, settings.ELASTICSEARCH_PASSWORD) if _has_auth else None,
        timeout=settings.ELASTICSEARCH_TIMEOUT
...
class TorResultsView(ElasticsearchBaseListView):
    ...
    def get_domain_profiles(self, hits):
        """Look up onion profiles for the domains present in the search hits."""
        domains = []
        for hit in hits:
            domain = hit.get('domain') or self._extract_domain(hit.get('url', ''))
            if domain:
                domains.append(domain)
        profile_repo = ProfileRepository()
        return profile_repo.get_profiles_for_domains(domains)
    ...
    def get_context_data(self, **kwargs):
        ...
        return {
            ...
            'domain_profiles': self.get_domain_profiles(self.object_list.hits),
        }
